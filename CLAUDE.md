...
## 19. 대시보드 데이터 미표시 및 중복 매장 버그 긴급 해결 (2026-07-16)

### 현상 및 이슈
- 로그인 완료 후 대시보드 진입 시 매출/매입 데이터가 노출되지 않고 "아직 데이터가 없습니다"가 표시되는 현상.
- 로그인 반복 과정에서 빈 매장(stores) 행이 중복(5개 이상) 누적 생성되는 현상.

### 원인 분석
- **스토어 ID 불일치**: 이전 로직에 임시 하드코딩되어 있던 구 테스트 매장 ID(`8de2930d-a196-4aa1-b9bf-7fa83321b10c`, **현재 DB에서 완전 삭제됨**)가 실제 라이브 매장(설맥현리점)의 매장 ID(`23197c4f-5bec-40e2-b494-826083721faa`)와 어긋나 401 Unauthorized 혹은 빈 데이터를 반환하고 있었음. 하드코딩은 이미 제거되었으며, 현재는 `stores.owner_id` 기반 DB 조회만 사용함.
- **조회 기준 분산**: 스토어 맵핑에 `user_id` 컬럼을 혼용 도입하려다 DB 과도기 스키마 및 세션 RLS 충돌이 발생함.
- **중복 생성 방지 미비**: 온보딩 데이터 인서트(`submitStore`) 시 동일 소유주(`owner_id`)의 스토어 존재 여부를 체크하지 않아 여러 번 세션이 마운트될 때 무분별하게 매장이 중복 인서트됨.

### 해결 조치
1. **스토어 조회 기준 단일화**:
   - `getStore.ts` 및 `middleware.ts` 등에서 하드코딩 ID 예외 조건 및 `user_id` 조회를 전면 제거하고, 오직 `owner_id = user.id` 기준 검색으로 일원화함.
2. **SQL 마이그레이션 폐기**:
   - 불필요한 스키마 변형 및 하드코딩 `UPDATE`문이 들어있던 `20260716_add_user_id_to_stores.sql` 파일을 삭제 처리함.
3. **중복 온보딩 생성 예외 차단**:
   - 온보딩 폼 제출 시 이미 `owner_id` 기준으로 할당된 스토어가 존재한다면 인서트 단계를 건너뛰고 바로 리다이렉트하는 사전 검증 로직을 탑재하여 중복 매장 생성을 예방함.

---

*업데이트: 2026-07-16*
*맥락: 대시보드 데이터 미표시 해결 및 소유권 owner_id 단일화*

---

## 20. 유료 모델 확정 및 관리자 구독 관리 구현 (2026-07-19)

### 주요 작업 내용
1. **초대코드 정리**: 
   - 가입 시 초대코드가 없는 전면 유료 모델 확정에 따라, 관리자 페이지 및 액션에서 초대코드 생성/조회/삭제 관련 기능 및 UI를 완전히 제거하였습니다.
   - 데이터베이스 `invite_codes` 테이블은 이력 보존을 위해 그대로 유지합니다.
2. **관리자 구독 관리 기능 추가**:
   - `/admin` 페이지에 가입된 유저들의 구독 정보를 보여주는 구독 관리 섹션을 추가했습니다.
   - 각 구독 정보의 이메일, 매장명, 플랜, 상태, 결제일, 만료일을 테이블로 제공합니다.
   - 만료일 및 상태를 연장할 수 있는 **[연장]** 기능을 추가하여 1/2/3/6개월 또는 직접 입력한 개월 수 만큼 수동으로 연장할 수 있게 했습니다. (만료된 구독은 오늘 날짜 기준으로 연장되며 상태는 `active`로 복구됩니다.)
3. **설정 API RLS 오류 해결**:
   - `app/api/stores/settings/route.ts`가 세션이 없는 `lib/supabase` 클라이언트를 사용해 RLS에 걸려 설정(사장님 급여 등) 저장/조회가 조용히 실패하던 오류를 해결하기 위해 `@/utils/supabase/server`의 `createClient`로 변경하였습니다.
   - 더 이상 사용되지 않는 `lib/supabase.ts` 파일을 삭제했습니다.
4. **미들웨어 하드코딩 제거**:
   - `utils/supabase/middleware.ts`의 `isAdmin` 확인 시 하드코딩되어 있던 `'kimtyy@gmail.com'` 이메일 폴백 조건을 제거하고 `process.env.ADMIN_EMAIL` 환경변수만 사용하도록 정비했습니다.
5. **잔재 파일 정리**:
   - `replace_store_id.js` 파일을 삭제하고 `check_db.js`를 `scripts/check_db.js`로 이동 정리했습니다.

*최종 업데이트: 2026-07-19*
*맥락: 초대코드 정리 + 구독 관리 기능 추가 + 설정 RLS 수정 + 미들웨어 정리*
